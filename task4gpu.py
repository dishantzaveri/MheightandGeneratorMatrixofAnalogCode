...
# Multi-Scale Exhaustive Random Sampling with optimized search space
def exhaustive_random_sampling(G, m, iterations=3, num_samples_per_iter=500_000, batch_size=10_000, scale_factor=10):
    k, n = G.shape
    best_x = None
    max_m_height = 0

    for scale in range(1, scale_factor + 1):
        bounds = (-1e5 * scale, 1e5 * scale)  # Reduced range for search space
...
def optimize_with_de(G, m, maxiter=500, popsize=30, scale=1e10, recombination=0.7, mutation=(0.6, 1.0)):
    k, n = G.shape
...
# Genetic Algorithm for Enhanced Global Search with reduced generations and population size
def genetic_algorithm(G, m, generations=500, population_size=100, mutation_rate=0.1, crossover_rate=0.8):
    k, n = G.shape
