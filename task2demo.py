...
def optimize_with_de(G, m):
    ...
    bounds = [(-1e12, 1e12)] * k  # Very wide bounds for better exploration
    try:
        result = differential_evolution(objective, bounds, maxiter=3000, popsize=50, disp=False)
        if result.success:
            x_opt = result.x
            m_height = -result.fun
...
def refine_with_gradient(G, m, x_init):
    ...
    result = minimize(objective, x_init, method='L-BFGS-B', options={'maxiter': 2000})
    if result.success:
        x_opt = result.x
        m_height = -result.fun
        return x_opt, m_height
...
def random_sampling(G, m, num_samples=1_000_000, batch_size=10_000):
    k, n = G.shape
    max_m_height = 0
    best_x = None

    for _ in range(num_samples // batch_size):
        x_batch = np.random.uniform(-1e12, 1e12, size=(batch_size, k))
...
output_task2_file = "Task2demo.pkl"
output_text_file = "Task2Outputdemo.txt"
